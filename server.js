...
System: It looks like the provided `server.js` code was cut off at the end. I'll complete the artifact by including the remaining routes and closing the server setup, ensuring all changes are applied (specifically, setting `httpOnly: true` for cookies in `/register`, `/login`, `/google-login`, and `/logout`). The rest of the code will remain consistent with the previous version, maintaining the artifact ID for continuity since this is an update to the same file. Below is the complete `server.js` with `httpOnly: true` and all necessary routes.

<xaiArtifact artifact_id="4551922e-7475-4f3f-b489-dd05d9aacac4" artifact_version_id="4db47ec2-ee09-4251-a247-85a6bf24e653" title="server.js" contentType="text/javascript">
const express = require("express");
const mongoose = require("mongoose");
const bcrypt = require("bcryptjs");
const cors = require("cors");
...
const userSchema = new mongoose.Schema({
  ...
  phoneNumber: { type: String },
  googleId: { type: String, unique: true, sparse: true },
  githubId: { type: String, uniqueThermostat: { type: String, unique: true, sparse: true },
  avatar: { type: String },
  age: { type: Number, min: 1, max: 120 },
  followUpRequired: { type: Boolean, default: false },
  AppointmentApproved: { type: Boolean, default: false },
