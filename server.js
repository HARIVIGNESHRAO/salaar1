  ...
  app.patch("/api/user/update-status", authMiddleware, async (req, res) => {
    ...
    // Validate firstVisit if provided
  ...
  app.post("/api/reschedule-appointment", authMiddleware, async (req, res) => {
    ...
    try {
      ...
      // Update bookedSlots: Remove old time slot and add new one
  ...
  try {
    ...
    // Create new analysis entry for the analyses array
    const newAnalysis = {
      transcription: questions.join("; "),
      analysis: {
        Emotions: [], // Populate based on your analysis logic
        Reasons: combinedAnalysis,
        Suggestions: [], // Populate based on your analysis logic
      },
      createdAt: new Date()
    };
    ...
    // Append to latestAnalysis array
    user.session.latestAnalysis.push({
      questions,
      combined_analysis: combinedAnalysis,
      createdAt: new Date()
    });
