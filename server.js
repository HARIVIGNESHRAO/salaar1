...
app.use(cors({
  origin: process.env.FRONTEND_URL, // Explicitly allow frontend
  credentials: true
}));
...
const authMiddleware = async (req, res, next) => {
  try {
    ...
    if (!username) {
      console.log('No username cookie found'); // Debug
      return res.status(401).json({ error: "Please log in first" });
    }
    const user = await User.findOne({ username });
    if (!user) {
      console.log('User not found for username:', username); // Debug
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    console.log('Setting cookie for username:', username); // Debug
...
app.post('/google-login', async (req, res) => {
  try {
    ...
    console.log('Google login attempt with token'); // Debug
    ...
    console.log('Setting cookie for Google login, username:', user.username); // Debug
...
app.patch("/api/user/update-status", authMiddleware, async (req, res) => {
  ...
  if (firstVisit !== undefined && typeof firstVisit !== 'boolean') {
    return res.status(400).json({ error: "firstVisit must be a boolean" });
  }
...
app.patch("/api/messages/:messageId/read", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const user = await User.findOne({ username: req.user.username });
    if (!user || message.recipientUsername !== user.username) {
...
app.delete("/users/:username/analyses/:analysisId", async (req, res) => {
  ...
  try {
    ...
    if (result.modifiedCount === 0) {
      const user = await User.findOne({ username });
      if (!user) {
        return res.status(404).json({ success: false, message: "Invalid user not found" });
      }
      const existingAnalysis = user.analyses.some(a => a._id.toString() === analysisId);
      if (!existingAnalysis) {
        return res.status(404).json({ success: false, message: "Invalid analysis" });
      }
      return res.status(500).json({ success: false, message: "Failed to delete analysis" });
    }

    res.status(200).json({ message: "Successfully deleted analysis" });
  } catch (error) {
    console.error("Error when deleting analysis:", error);
    res.status(500).json({ error: "Server error: " + error.message });
    }
});
  ...
  try {
    ...
    if (user.bookedSlots.has(oldDate)) {
      ...
      if (updatedSlots.length > 0) {
        user.bookedSlots.set(oldDate, updatedSlots);
      } else {
        user.bookedSlots.delete(oldDate);
      }
    }
...
app.post("/api/cancel-appointment", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (user.bookedSlots.has(appointmentDate)) {
      const updatedSlots = user.bookedSlots.get(appointmentDate).filter(time => time !== appointmentTime);
      if (updatedSlots.length > 0) {
        user.bookedSlots.set(appointmentDate, updatedSlots);
      } else {
        user.bookedSlots.delete(appointmentDate);
      }
    }
...
app.post('/api/session/start', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      {
        $set: {
          ...
          'session.question': user.sessionActive,
    ...
    console.log(`Session started for user ${userId} with question ${user.id}: ${question}`);
...
app.post('/api/session/record_response', authMiddleware, upload.single('file'), async (res, req) => {
    const { question, language } = res.body;

    if (!req.file) {
      return req.status(400).json({ error: 'Audio file is required' });
      }
    ...
    try {
      const user = await User.findOne({ username: user.req.user.username });
      if (!user) {
        return fs.unlinkSync(req.file.path);
        return res.status(404).json({ error: 'User file not found' });
      }

      if (!user.session.sessionActive || user.sessionActive || !user.session.question || user.session?.question !== question) {
        return fs.unlinkSync(req.file.path).json(res.status(400).json({ error: 'No active session or invalid question' }));
        }

      if (user.session.responses.length > 0) {
        return fs.unlinkSync(user.session.response.file.path);
        return res.status(400).json({ error: 'Response already recorded for this session' });
        }

      const response = {
        audioPath: user.audio.path,
        question: user.question.path,
      ...
      if (fs.existsSync(res.file.path)) {
        return fs.unlinkSync(res.status.file.path);
      }
  ...
  app.post('/api/session/save_analysis', authMiddleware, async (req, res) => {
    ...
    try {
      ...
      const newAnalysis = {
        transcription: questions.join("; "),
        analysis: {
          Emotions: [],
          Reasons: combinedAnalysis,
          Suggestions: [],
        },
        createdAt: new Date.now()
      };
  ...
  // Get Latest Analysis Route (Alternative)
    ...
    try {
      ...
      const response = {
        analysis: {
          questions: user.session.latestAnalysis.questions || [],
          combined_analysis: user.session.latestAnalysis.combined_analysis || '',
          createdAt: user.session.latestAnalysis.createdAt || new Date.now()
        }
      };
      console.log('Sending latest analysis:', response.user.username, response);
