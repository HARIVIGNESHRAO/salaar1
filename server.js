...
// Check User Cookie Middleware
const checkUserCookie = async (req, res, next) => {
  try {
...
// Save Analysis Route
app.post('/save_analysis', checkUserCookie, async (req, res) => {
  try {
...
// Get Latest Analysis
app.get('/users/username/latest_analysis', checkUserCookie, async (req, res) => {
  try {
...
app.delete('/users/username/latest_analysis', checkUserCookie, async (req, res) => {
  try {
    const user = await User.findOne({ username: req.user.username });
    if (!user || !user.session || !user.session.latestAnalysis) {
      return res.status(404).json({ success: false, message: 'No latest analysis found' });
...
app.post("/users/:username/export-analyses", checkUserCookie, async (req, res) => {
...
app.delete("/users/:username/analyses/:analysisId", checkUserCookie, async (req, res) => {
...
// Delete User Route
app.delete("/users/:id", checkUserCookie, async (req, res) => {
...
app.patch("/users/:id", checkUserCookie, async (req, res) => {
  const { id } = req.params;
...
// Profile Routes
// Get User Profile
app.get("/api/user/profile", checkUserCookie, async (req, res) => {
  try {
    const user = await User.findOne({ username: req.user.username })
...
// Update User Profile
app.put("/api/user/profile", checkUserCookie, async (req, res) => {
...
// Send SMS Route
app.post("/api/send-sms", checkUserCookie, async (req, res) => {
...
app.patch("/api/user/approve-appointment", checkUserCookie, async (req, res) => {
...
app.post("/api/reschedule-appointment", checkUserCookie, async (req, res) => {
...
app.post("/api/cancel-appointment", checkUserCookie, async (req, res) => {
...
// Get Active Session
app.get('/api/session/active', checkUserCookie, async (req, res) => {
  try {
    const user = await User.findOne({ username: req.user.username })
      .select('session');
    if (!user) {
...
// Start Session
app.post('/api/session/start', checkUserCookie, async (req, res) => {
...
app.post('/api/session/record_response', checkUserCookie, upload.single('file'), async (req, res) => {
...
app.get('/api/session/responses/:userId', checkUserCookie, async (req, res) => {
...
app.post('/api/session/save_analysis', checkUserCookie, async (req, res) => {
...
app.get('/api/session/latest_analysis', checkUserCookie, async (req, res) => {
  try {
    const user = await User.findOne({ username: req.user.username })
      .select('session.latestAnalysis');
    if (!user) {
...
app.get('/api/session/latest_analysis1', checkUserCookie, async (req, res) => {
  try {
    const user = await User.findOne({ username: req.user.username })
      .select('session.latestAnalysis');
    if (!user) {
...
// End Session
app.post('/api/session/end', checkUserCookie, async (req, res) => {
